...
#define VERSION_STR    "tokapps-0.01"
const char FullVersionStr[] = VERSION_STR " compiled at " __DATE__ " " __TIME__ " by gcc "
__VERSION__ " "
#ifdef __LP64__
"64-bit"
#else
...
static inline int ext_conn_hash(int in_fd, long long in_conn_id) {
    unsigned long long h = (unsigned long long) in_fd * 11400714819323198485ULL +
                           (unsigned long long) in_conn_id * 13043817825332782213ULL;
...
struct ext_connection *
get_ext_connection_by_in_conn_id(int in_fd, int in_gen, long long in_conn_id, int mode, int *created) {
    check_engine_class();
...
// MUST be new
struct ext_connection *
create_ext_connection(connection_job_t CI, long long in_conn_id, connection_job_t CO, long long auth_key_id) {
    check_engine_class();
    struct ext_connection *Ex = get_ext_connection_by_in_conn_id(CONN_INFO(CI)->fd, CONN_INFO(CI)->generation,
                                                                 in_conn_id, 2, 0);
...
#define SW(x)    (workers ? S1(x) : S(x))
    sb_printf(sb,
              ...
              "proxy_mode\t%d\n"
              "proxy_tag_set\t%d\n"
              "version\t" VERSION_STR " compiled at " __DATE__ " " __TIME__ " by gcc "
    __VERSION__
    " "
    #ifdef __LP64__
    "64-bit"
...
int callback_job_run(job_t job, int op, struct job_thread *JT) {
    ...
    switch (op) {
        case JS_RUN:
            return D->func(D->data, job->j_custom_bytes - offsetof(
            struct job_callback_info, data));
...
void schedule_job_callback(int context, job_callback_func_t func, void *data, int len) {
    job_t
    job = create_async_job(callback_job_run, JSP_PARENT_RWE | JSC_ALLOW(context, JS_RUN) | JSIG_FAST(JS_FINISH), -2,
                           offsetof(
    struct job_callback_info, data) +len, 0, JOB_REF_NULL);
    assert(job);
    struct job_callback_info *D = (struct job_callback_info *) (job->j_custom);
    D->func = func;
        ...
        case RPC_PROXY_ANS:
            if (len >= 16) {
                ...
                vkprintf(2, "got RPC_PROXY_ANS from connection %d:%llx, data size = %d, flags = %d\n", CONN_INFO(C)->fd,
                         out_conn_id, tl_fetch_unread(), flags);
...
int rpcc_execute(connection_job_t C, int op, struct raw_message *msg) {
    ...
    switch (op) {
        ...
        case RPC_SIMPLE_ACK:
        case RPC_CLOSE_EXT: {
            job_t job = create_async_job(client_packet_job_run, JSP_PARENT_RWE | JSC_ALLOW(JC_ENGINE, JS_RUN) |
                                                                JSC_ALLOW(JC_ENGINE, JS_ABORT) |
                                                                JSC_ALLOW(JC_ENGINE, JS_ALARM) |
                                                                JSC_ALLOW(JC_ENGINE, JS_FINISH), -2,
                                         sizeof(struct client_packet_info), JT_HAVE_TIMER, JOB_REF_NULL);
            struct client_packet_info *D = (struct client_packet_info *) (job->j_custom);
            D->msg = *msg;
            D->type = op;
            D->conn = job_incref(C);
...
int
forward_mtproto_packet(struct tl_in_state *tlio_in, connection_job_t C, int len, int remote_ip_port[5], int rpc_flags);

int forward_tcp_query(struct tl_in_state *tlio_in, connection_job_t C, conn_target_job_t S, int flags,
                      long long auth_key_id, int remote_ip_port[5], int our_ip_port[5]);
...
int process_http_query(struct tl_in_state *tlio_in, job_t HQJ) {
    ...
    if (qUriLen >= 4 && !memcmp(qUri, "/api", 4)) {
        ...
        if (D->query_type == htqt_options) {
            ...
            int len = snprintf(response_buffer, 511,
        ...
        cur_http_origin_len = get_http_header(qHeaders, qHeadersLen, cur_http_origin, sizeof(cur_http_origin) - 1,
                                              "Origin", 6);
        cur_http_referer_len = get_http_header(qHeaders, qHeadersLen, cur_http_referer, sizeof(cur_http_referer) - 1,
                                               "Referer", 7);
        cur_http_user_agent_len = get_http_header(qHeaders, qHeadersLen, cur_http_user_agent,
        ...
        if ((CONN_INFO(c)->remote_ip & 0xff000000) == 0x0a000000 ||
            ...
            int x_real_ip_len = get_http_header(qHeaders, qHeadersLen, x_real_ip, sizeof(x_real_ip) - 1, "X-Real-IP",
                                                9);
            int x_real_port_len = get_http_header(qHeaders, qHeadersLen, x_real_port, sizeof(x_real_port) - 1,
                                                  "X-Real-Port", 11);
            if (x_real_ip_len > 0) {
                ...
                if (real_ip >= (1 << 24) || parse_text_ipv6((unsigned char *) tmp_ip_port, x_real_ip) > 0) {
                    ...
                    vkprintf(3,
                             "set remote IPv6:port to %08x:%08x:%08x:%08x:%08x according to X-Real-Ip '%s', X-Real-Port '%s'\n",
                             tmp_ip_port[0], tmp_ip_port[1], tmp_ip_port[2], tmp_ip_port[3], tmp_ip_port[4], x_real_ip,
...
int http_query_job_run(job_t job, int op, struct job_thread *JT) {
    ...
    switch (op) {
        ...
        case JS_FINISH: // NET-CPU
            if (HQ->flags & 1) {
                connection_job_t c = HQ->conn ? job_incref(HQ->conn) : connection_get_by_fd_generation(HQ->conn_fd,
                                                                                                       HQ->conn_generation);
                if (c) {
                    assert(CONN_INFO(c)->pending_queries == 1);
                    CONN_INFO(c)->pending_queries--;
...
int hts_execute(connection_job_t c, struct raw_message *msg, int op) {
    ...
    job_t job = create_async_job(http_query_job_run,
                                 JSP_PARENT_RWE | JSC_ALLOW(JC_ENGINE, JS_RUN) | JSC_ALLOW(JC_ENGINE, JS_ABORT) |
                                 JSC_ALLOW(JC_ENGINE, JS_ALARM) | JSC_ALLOW(JC_CONNECTION, JS_FINISH), -2,
                                 sizeof(struct http_query_info) + D->header_size + 1, JT_HAVE_TIMER, JOB_REF_NULL);
    assert(job);
    struct http_query_info *HQ = (struct http_query_info *) (job->j_custom);
        ...
        TLS_START_UNALIGN (JOB_REF_CREATE_PASS(C))
            {
                ...
                tl_store_raw_data(response_buffer, snprintf(response_buffer, sizeof(response_buffer) - 1,
                                                            "HTTP/1.1 200 OK\r\nConnection: %s\r\nContent-type: application/octet-stream\r\nPragma: no-cache\r\nCache-control: no-store\r\n%sContent-length: %d\r\n\r\n",
                                                            (D->query_flags & QF_KEEPALIVE) ? "keep-alive" : "close",
                                                            D->query_flags & QF_EXTRA_HEADERS
                                                            ? mtproto_cors_http_headers : "", len));
...
static int forward_mtproto_enc_packet(struct tl_in_state *tlio_in, connection_job_t C, long long auth_key_id, int len,
                                      int remote_ip_port[5], int rpc_flags) {
    if (len < offsetof(
        struct encrypted_message, message) /*|| (len & 15) != (offsetof (struct encrypted_message, server_salt) & 15)*/) {
        return 0;
    }
    vkprintf(2, "received mtproto encrypted packet of %d bytes from connection %p (#%d~%d), key=%016llx\n", len, C,
             CONN_INFO(C)->fd, CONN_INFO(C)->generation, auth_key_id);
...
forward_mtproto_packet(struct tl_in_state *tlio_in, connection_job_t C, int len, int remote_ip_port[5], int rpc_flags) {
    ...
    if (function != CODE_req_pq && function != CODE_req_pq_multi && function != CODE_req_DH_params &&
...
int forward_tcp_query(struct tl_in_state *tlio_in, connection_job_t c, conn_target_job_t S, int flags,
    ...
    vkprintf(3,
             "forwarding user query from connection %d~%d (ext_conn_id %llx) into connection %d~%d (ext_conn_id %llx)\n",
...
void check_all_conn_buffers(void) {
    ...
    while (to_free > 0 && ConnLRU.lru_next != &ConnLRU) {
        ...
        vkprintf(2,
                 "check_all_conn_buffers(): closing connection %d because of %lld total used buffer vytes (%lld max, %lld bytes to free)\n",
        ...
        if (d) {
            int tot_used_bytes =
                    CONN_INFO(d)->in.total_bytes + CONN_INFO(d)->in_u.total_bytes + CONN_INFO(d)->out.total_bytes +
                    CONN_INFO(d)->out_p.total_bytes;
...
int check_conn_buffers(connection_job_t c) {
    int tot_used_bytes = CONN_INFO(c)->in.total_bytes + CONN_INFO(c)->in_u.total_bytes + CONN_INFO(c)->out.total_bytes +
                         CONN_INFO(c)->out_p.total_bytes;
...
void mtfront_pre_loop(void) {
    ...
    if (!workers) {
        for (i = 0; i < http_ports_num; i++) {
            init_listening_tcpv6_connection(http_sfd[i], &ct_tcp_rpc_ext_server_mtfront, &ext_rpc_methods,
...
void mtfront_prepare_parse_options(void) {
    ...
    parse_option("slaves", required_argument, 0, 'M',
                 "spawn several slave workers; not recommended for TLS-transport mode for better replay protection");
...
void mtfront_pre_init(void) {
    ...
    if (workers) {
        ...
        WStats = mmap(0, 2 * workers * sizeof(struct worker_stats), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
                      -1, 0);
        assert(WStats);
