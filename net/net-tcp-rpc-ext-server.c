...
int tcp_proxy_pass_connected(connection_job_t C) {
    ...
    vkprintf(1, "proxy pass connected #%d %s:%d -> %s:%d\n", c->fd, show_our_ip(C), c->our_port, show_remote_ip(C),
             c->remote_port);
    return 0;
}
...
static unsigned char ext_secret[4000][16];//tokapps
static unsigned char secretConnectionCount[4000][3];//created by tokapps.ir
static int ext_secret_cnt = 0;

void tcp_rpcs_set_ext_secret(unsigned char secret[4000]) {//tokapps
    assert(ext_secret_cnt < 4000);
    memcpy(ext_secret[ext_secret_cnt++], secret, 16);//tokapps
}
...
struct domain_info {
    ...
    unsigned char target_ipv6[16];//tokapps
...
static int check_response(const unsigned char *response, int len, const unsigned char *request_session_id,
                          int *is_reversed_extension_order, int *encrypted_application_data_length) {
#define FAIL(error) {                                               \
...
#define TRIES 20
                ...
                if (responses[i] == NULL) {
                    ...
                    if (read_res != sizeof(header)) {
                        kprintf("Failed to read response header for checking domain %s: %s\n", domain,
                                read_res == -1 ? strerror(errno) : "Read less bytes than expected");
                        have_error = 1;
                        break;
                    }
                    ...
                    if (read_pos[i] == response_len[i]) {
                        if (!is_encrypted_application_data_length_read[i]) {
                            if (memcmp(responses[i] + response_len[i] - 11, "\x14\x03\x03\x00\x01\x01\x17\x03\x03",
                                       9) != 0) {
                            ...
                            int encrypted_application_data_length =
                                    responses[i][response_len[i] - 2] * 256 + responses[i][response_len[i] - 1];
                        ...
                        if (check_response(responses[i], response_len[i], requests[i] + 44,
                                           &is_reversed_extension_order, &encrypted_application_data_length)) {
            ...
            if (FD_ISSET(sockets[i], &write_fd)) {
                ...
                if (write_res != TLS_REQUEST_LENGTH) {
                    kprintf("Failed to write request for checking domain %s: %s", domain,
                            write_res == -1 ? strerror(errno) : "Written less bytes than expected");
                    have_error = 1;
                    break;
                }
                is_written[i] = 1;
    ...
    if (encrypted_application_data_length_min == encrypted_application_data_length_max) {
        ...
        kprintf("Unrecognized encrypted application data length pattern with min = %d, max = %d, mean = %.3lf\n",
                encrypted_application_data_length_min, encrypted_application_data_length_max,
    ...
    vkprintf(0,
             "Successfully checked domain %s in %.3lf seconds: is_reversed_extension_order = %d, server_hello_encrypted_size = %d, use_random_encrypted_size = %d\n",
...
static int proxy_connection(connection_job_t C, const struct domain_info *info) {
    ...
    job_t EJ = alloc_new_connection(cfd, NULL, NULL, ct_outbound, &ct_proxy_pass, C,
                                    ntohl(*(int *) &info->target.s_addr), (void *) info->target_ipv6, port);
    ...
    while (1) {
        ...
        return tcp_rpcs_parse_execute (C);
            ...
            if (c->flags & C_IS_TLS) {
                ...
                c->left_tls_packet_length =
                        256 * header[9] + header[10]; // store left length of current TLS packet in extra_int3
                ...
                int secret_id;
                //      tokapps Parse secret Code Connection
                ...
                if (cipher_suites_length <= 1 || client_hello[pos] != 0x13 || client_hello[pos + 1] < 0x01 ||
            ...
            if (allow_only_tls && !(c->flags & C_IS_TLS)) {
                ...
                RETURN_TLS_ERROR(default_domain_info); //tokapps commented
            }
      ...
      if (!tmp[1] && !(c->flags & C_IS_TLS)) {
        ...
        return tcp_rpcs_parse_execute (C);
            ...
            if (ext_secret_cnt > 0) {
                vkprintf(1, "invalid \"random\" 64-byte 1 header, entering global skip mode ext_secret_cnt is: %d and secret code is %s \n",ext_secret_cnt,ext_secret[secret_id]);
                return (-1 << 28);
            }
